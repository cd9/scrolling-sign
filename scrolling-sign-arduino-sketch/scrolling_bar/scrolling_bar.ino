...
// TODO:Ser Subtract the offset from the char before starting the send sequence to save time if nessisary
...
static inline void sendChar( uint8_t c ,  uint8_t skip) {
  ...
  while (col--) {
      sendRowRGB( pgm_read_byte_near( charbase++ ) , red , green , blue );
  }    
  ...
  while (col--) {
    ...
    sendRowRGB( 0 , red , green , blue );    // Interchar space
...
static inline void sendString( const char *s , uint8_t skip) {

  unsigned int l=PIXELS/(FONT_WIDTH+INTERCHAR_SPACE); 

  sendChar( *s , skip);   // First char is special case becuase it can be stepped for smooth scrolling
...
SoftwareSerial piSerial =  SoftwareSerial(11, 12);
  ...
  if (gotmessage){
    while (*m) {      
        ...
        for( uint8_t step=0; step<FONT_WIDTH+INTERCHAR_SPACE  ; step++ ) {   // step though each column of the 1st char for smooth scrolling
           ...
           sendString( m , step , red, green, blue );    // Nice and not-too-bright blue hue
          
           sei();
          
           delay(15);   // Slows down scrolling by pausing 10 milliseconds between horizontal steps         
...
/*char* serialString()
{
  ...
  static char str[600]; // For strings of max length=301
  if (!Serial.available()) {
  ...
  str[count*4]='\0'; // make it a zero terminated string
  return str;
}*/
